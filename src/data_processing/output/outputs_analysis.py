...
def long_run(info_runs, gen_crimes, model_params, gen_neighborhoods, neighborhoods):
    gen_crimes.reset_index(inplace = True)
    info_runs.reset_index(inplace = True)
    info_long_runs = info_runs
    
    #Plotting all crimes
    #plot_gen_crimes(gen_crimes, neighborhoods, gen_neighborhoods, "all")
    ...
    info_long_runs = info_long_runs.merge(gen_crimes, on = 'run_id', how = 'left')
    
    #Add metrics: analysis on time
    def time_analysis():
        parameters = np.log(info_long_runs[['PoliceAgent_p_agents', 'Pickpocket_p_agents', 'Robber_p_agents', 'p_information_Worker', 'opportunity_awareness_Criminal', 'crowd_effect_Criminal', 'day']])
        outcomes = ['crimerate', 'psuccessful', 'probberies', 'ppickpocketing']
        regr = linear_model.LinearRegression()
        for outcome in outcomes:
            x = parameters.loc[info_long_runs[outcome].notna(), :]
            y = np.log(info_runs.loc[info_long_runs[outcome].notna(), outcome])
            #sensitivity_df[outcome] = (regr.coef_*100).round(2)
            regr.fit(x, y)
            conf_int = _regression_conf_int(0.05, regr, x, y)
            conf_int = conf_int.iloc[1:]
    ...
    #info_long_runs = info_long_runs[info_long_runs['run_id'].isin(long_run_models)]
    ...
    info_long_runs = info_long_runs.merge(info_long_runs[['victim_recurrence', 'run_id']].groupby('run_id').apply(_gini).rename('gini_victim'), on = 'run_id')
    ...
    performance_metrics = ['silhouette_score', 'daily_neighborhood_variance', 'gini_offender', 'gini_criminal']
    info_long_runs = info_long_runs[performance_metrics].iloc[0]
    
    _save_table(info_long_runs, "info_long_runs")
    return info_long_runs

def plot_gen_crimes(gen_crimes, neighborhoods, gen_neighborhoods, name):
    gen_neighborhoods.reset_index(inplace = True)
    info_neighborhoods = pd.DataFrame()
    info_neighborhoods['crimes_per_neighborhood'] = gen_neighborhoods[['run_crimes', 'neighborhood_id']].groupby('neighborhood_id').sum()
    info_neighborhoods['total_crimes'] = gen_neighborhoods['run_crimes'].sum()
    info_neighborhoods['percentage'] = info_neighborhoods['crimes_per_neighborhood']/info_neighborhoods['total_crimes']*100
    info_neighborhoods['percentage'] = info_neighborhoods['percentage'].astype(float).round(2)
    info_neighborhoods = gpd.GeoDataFrame(info_neighborhoods.merge(neighborhoods[['geometry']], left_on = 'neighborhood_id', right_index = True))
    plot = info_neighborhoods.plot(column='percentage', 
                                cmap = 'OrRd', 
                                legend=True, 
                                legend_kwds={'label': "Percentage on total generated crimes (%)"},
                                missing_kwds={'color': 'lightgrey'})
    gen_crimes[gen_crimes['position'].notnull()].plot(ax = plot, color = 'black', markersize = 0.5, alpha = 0.2)
...
def _save_table(df, name, rounded = None, percentages = None, integers = None):
    ...
    if isinstance(df, pd.DataFrame):
        ...
        df = df.rename(lambda x: x.replace("_" , "") if "_" in x else x)
    df.to_csv(f"outputs/tables/{name}.csv", quoting=csv.QUOTE_NONE)
...
def main():
    ...
    long_run_df = long_run(info_runs, gen_crimes, model_params, gen_neighborhoods, neighborhoods)
