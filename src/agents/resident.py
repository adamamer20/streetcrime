...
class Resident(InformedMover):
    ...
    attributes: dict[str, int or list(dt.datetime)] = {
        ...
        'resting_start_time' : "_gen_attribute('resting_start_time', attribute_type = 'datetime_variable')",
        'resting_end_time' : "_gen_attribute('resting_end_time', attribute_type = 'datetime_variable')",
        'income' : "_gen_attribute('income')"
        }
    ...
    def step(self) -> None:
        ...
        if self.model.data['datetime'].time() == time(hour = 14, minute = 0):
            self._initialize_attributes(['resting_start_time', 'resting_end_time'])
