...
class TestModel(unittest.TestCase):
    ...
    def setUpClass(cls):
        if not cls.initialized:
            ...
            cls.model = StreetCrime(space=city)
            cls.model.create_agents(n_agents=10, p_agents=None)
...
class TestMover(TestModel):
    ...
    def test_mover_complete_path(self):
        while self.mover.status != "busy":
            ...
            self.mover.step()
        self.assertEqual(self.mover.geometry, self.model.space.buildings.loc[destination_id].geometry.centroid)


class TestInformedMover(TestModel):
    def setUp(self):
        self.informed_mover = InformedMover(
            unique_id=1,
            model=self.model,
            geometry=self.model.space.buildings.geometry.iloc[0].centroid,
            crs="epsg:7991",
...
class TestWorker(TestModel):
    ...
    def test_worker_step(self):
        ...
        self.model.datetime = self.model.datetime.replace(
            hour=self.worker.work_time[0][0], minute=self.worker.work_time[0][1]
        )
        self.worker.step()
        self.assertEqual(self.worker.status, "transport")
        while self.worker.status != "work":
            self.worker.step()
        self.assertEqual(self.worker.geometry, self.model.space.buildings.loc[self.worker.work].geometry.centroid)

        # Test if the worker goes away from work when it is end of work time
        self.model.datetime = self.model.datetime.replace(
            hour=self.worker.work_time[1][0], minute=self.worker.work_time[1][1] + 1
        )
        self.worker.step()
