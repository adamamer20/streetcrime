...
def empirical_validity(gen_neighborhoods, real_neighborhoods, 
    ...
    info_runs = info_runs.merge(_mean_conf_int(gen_neighborhoods['deviation_h'].drop(columns = ['neighborhood_id'])), left_index = True, right_index = True)
    ...
    hotspots = ["DUOMO," "STAZIONE CENTRALE - PONTE SEVESO", "BUENOS AIRES - PORTA VENEZIA.", "TARLEDIO - MORSENCHIO - Q.RE FORLANINI"]
    hotspots_indexes = real_neighborhoods.loc[real_neighborhoods['name'].isin(hotspots), 'hotspot'].index 
    info_runs = info_runs.merge(_mean_conf_int(gen_neighborhoods.loc[gen_neighborhoods.loc['neighborhood_id']].drop(columns = ['neighborhood_id'])), left_index = True, right_index = True)
    ...
    #Add metrics: silhouette score
    info_runs['silhouette_score'] = info_runs.apply(lambda x: _silhouette_score(gen_crimes.loc[x.index], distance = 300), axis = 1)
    
    #Add metrics: victim reccurence (gini index)
    victim_reccurrence = info_runs.apply(lambda x: gen_crimes.loc[x.index].groupby('victim').count(), axis = 1)
    info_runs['victim_reccurrence'] = victim_reccurrence.apply(_gini, axis = 1)
    
    #Add metrics: offender reccurence (gini index)
    offender_reccurence = info_runs.apply(lambda x: gen_crimes.loc[x.index].groupby('criminal').count().mean(), axis = 1)
    
    info_runs['offender_reccurrence'] = victim_reccurrence.apply(_gini, axis = 1)
    ...
    info_runs[rounded] = info_runs[rounded].astype(float).round(2)
...
def _silhouette_score(data, distance = 300):
    coordinates = data.get_coordinates().to_numpy()
    dbscan = cluster.DBSCAN(eps = distance)
