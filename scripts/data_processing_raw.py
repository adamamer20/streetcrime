...
#SHOULD CONVERT DATA PROCESSING TO A CLASS

def roads_data_processing(raw_file : str,
                          out_file: str) -> gpd.GeoDataFrame:
    roads_df = gpd.read_file(raw_file) 
    roads_df.geometry = convert_3D_2D(roads_df.geometry) # new geodf with 2D geometry series
    roads_df.geometry = segmented(roads_df.geometry) # new geodf with segmented lines
    roads_df.to_file(out_file)
    return roads_df
...
def convert_3D_2D(geometry):
    ...
    for line in geometry:
            coordinates = [xy[:2] for xy in list(line.coords)]
            new_line = LineString(coordinates)
...
def buildings_data_processing(raw_file : str, 
    ...
    buildings_fun_dbf = Dbf5(fun_file)
    ...
    buildings_df = buildings_df.sample(n=2, random_state=1)
    ...
    #Convert position and entrance tuple to string (to save the file)
    buildings_df['position'] = buildings_df['position'].apply(lambda x: '('+','.join(map(str, x))+')') #Maybe it is useless in further code
    buildings_df['entrance'] = buildings_df['entrance'].apply(lambda x: '('+','.join(map(str, x))+')')
...
def set_building_entrance(buildings_df: gpd.GeoDataFrame, roads: RoadNetwork) -> None: #TO DO:this should be done in the data processing (otherwise it would do it everytime we run the model) 
    buildings_df["position"] = [
        (x, y) for x, y in zip(buildings_df.centroid.x, buildings_df.centroid.y)] #We convert it to a tuple to use the function get_nearest_node()
    buildings_df['entrance'] = [
        roads.get_nearest_node(x) for x in buildings_df['position']]
    return buildings_df

def main():
    start_time = time.time()
    #Processing roads roads: --- 58.210232973098755 seconds ---
    roads_raw_file = os.path.join(parent_directory, r"data\raw\DBT_2020\DBT 2020 - SHAPE\EL_STR.shp")
    roads_out_file = os.path.join(parent_directory, r"data\processed\roads.shp") 
    if os.path.isfile(roads_out_file) == True:
        roads_df = gpd.read_file(roads_out_file) 
    else:
        roads_df = roads_data_processing(roads_raw_file, roads_out_file)
    roads = RoadNetwork(roads_df.geometry)
